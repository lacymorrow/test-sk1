...
// Move data to a constant with type
...
const TIMER_RESET = 10000;

export const FeaturesTimed = () => {
  const [featureOpen, setFeatureOpen] = useState<number>(0);
  const [timer, setTimer] = useState<number>(0);

  useEffect(() => {
    const interval = setInterval(() => {
      setTimer((prev) => prev + TIMER_INCREMENT);
    }, TIMER_INTERVAL);
  ...
  useEffect(() => {
    if (timer > TIMER_RESET) {
      setFeatureOpen((prev) => (prev + 1) % FEATURE_DATA.length);
      setTimer(0);
    }
  }, [timer]);
          ...
          >
            {FEATURE_DATA.map((item, index) => (
              <img
                ...
                key={item.title}
                src={item.srcImage}
                style={{ zIndex: FEATURE_DATA.length - index }}
              />
            ))}
          </div>
        </div>
...
// Separate interface for TextComponent props
