...
export class MetricsService {
	...
	async recordMetric(
		...
		try {
			await redis!.zadd(`${this.prefix}:${name}`, metric.timestamp, JSON.stringify(metric));
		} catch (error) {
	...
	async getMetrics(name: string, query: MetricQuery = {}): Promise<MetricData[]> {
		...
		try {
			const metrics = await redis!.zrangebyscore(`${this.prefix}:${name}`, from, to);

			return metrics.map((metric) => JSON.parse(metric) as MetricData).slice(0, limit);
		} catch (error) {
