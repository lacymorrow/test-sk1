...
export function useMultiAsyncState<T extends Record<string, unknown>>() {
  const [states, setStates] = useState<Record<keyof T, AsyncState<unknown>>>({} as Record<keyof T, AsyncState<unknown>>);

  const createExecutor = useCallback(<K extends keyof T>(
    key: K,
  ...
  const getState = useCallback(<K extends keyof T>(key: K): AsyncState<T[K]> => {
    return states[key] as AsyncState<T[K]> || { data: null, loading: false, error: null, success: false };
  }, [states]);
